...
export class WalletService {
  ...
  async getAccountBalances(publicKey: string): Promise<AssetBalance[]> {
    ...
    try {
      account = await this.server.loadAccount(publicKey) as unknown as StellarAccount;
    } catch (err: unknown) {
      ...
      const message = isStellarError(err) && err.message ? err.message : 'Unknown error';
    ...
    const balances: AssetBalance[] = account.balances
      .filter((b: StellarBalance) => {
        ...
        if (
          ...
          return b.asset_code !== undefined && (SUPPORTED_ASSETS as readonly string[]).includes(b.asset_code);
        }
        return false;
      })
